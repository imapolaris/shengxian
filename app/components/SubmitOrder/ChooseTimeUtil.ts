...
export const LeftData = ()=>{
    return leftAllValue.map((item:any,i:number)=>{
        return item.label+'('+GetDateStr(i)+')'
    });
};

//右侧显示的数据
export const RightData = (state:string)=>{
    if (state === 'today'){
...
export const PickData = ()=>{
    let left:Array<any> =  LeftData();
    let right:Array<any> =  RightData('today');
    let right1:Array<any> =  RightData('not today');
...
export function GetSelectedDate(left:string,right:string){
    let add = 0;
    if(_.startsWith(left,"今天")) add = 0;
    else if(_.startsWith(left,"明天")) add = 1;
    else if(_.startsWith(left,"后天")) add = 2;
...
export function formartSelectedDate(date:Date){
    let now  =moment();
    let dm = moment(date);
...
export function autoPeisongDate(date:Date|string|null){
    if(date != null){
...
export function calcFirstPeisongDate(){
    let now  =moment().add(30,"m");
